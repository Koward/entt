...
class basic_view<Entity, exclude_t<Exclude...>, Component...> {
    ...
    using view_type = view_t<Entity, Comp>;

    template<typename Comp>
    using component_iterator = decltype(std::declval<view_type<Comp>>().begin());
    ...
    class view_range {
        ...
        using range_view_iterator = view_iterator<typename basic_sparse_set<Entity>::iterator>;

        class range_iterator {
            friend class view_range;

            using ref_type = decltype(std::tuple_cat(std::declval<std::conditional_t<is_eto_eligible_v<Component>, std::tuple<>, std::tuple<view_type<Component> *>>>()...));

            range_iterator(range_view_iterator from, ref_type ref) ENTT_NOEXCEPT
                : it{from},
                  pools{ref}
            {}
        ...
        private:
            range_view_iterator it{};
            const ref_type pools{};
        };

        view_range(range_view_iterator from, range_view_iterator to, std::tuple<view_type<Component> *...> ref)
            : first{from},
              last{to},
              pools{ref}
        {}
    ...
    public:
        ...
        [[nodiscard]] iterator begin() const ENTT_NOEXCEPT {
            ...
            }(std::get<view_type<Component> *>(pools))...)};
        }
    ...
    private:
        range_view_iterator first;
        range_view_iterator last;
        const std::tuple<view_type<Component> *...> pools;
    };

    basic_view(view_type<Component> &... component, unpack_as_t<const basic_sparse_set<Entity>, Exclude> &... epool) ENTT_NOEXCEPT
        : pools{&component...},
          view{candidate()},
          filter{&epool...}
    {}
    ...
    [[nodiscard]] const basic_sparse_set<Entity> * candidate() const ENTT_NOEXCEPT {
        return (std::min)({ static_cast<const basic_sparse_set<Entity> *>(std::get<view_type<Component> *>(pools))... }, [](const auto *lhs, const auto *rhs) {
    ...
    [[nodiscard]] unchecked_type unchecked(const basic_sparse_set<Entity> *cpool) const {
        ...
        ((std::get<view_type<Component> *>(pools) == cpool ? nullptr : (other[pos] = std::get<view_type<Component> *>(pools), other[pos++])), ...);
        return other;
    }

    template<typename Comp, typename Other>
    [[nodiscard]] decltype(auto) get([[maybe_unused]] component_iterator<Comp> &it, [[maybe_unused]] view_type<Other> *cpool, [[maybe_unused]] const Entity entt) const {
    ...
    void traverse(Func func, type_list<Type...>) const {
        if constexpr(std::disjunction_v<std::is_same<Comp, Type>...>) {
            auto it = std::get<view_type<Comp> *>(pools)->begin();

            for(const auto entt: static_cast<const basic_sparse_set<entity_type> &>(*std::get<view_type<Comp> *>(pools))) {
                if(((std::is_same_v<Comp, Component> || std::get<view_type<Component> *>(pools)->contains(entt)) && ...)
                        && (sizeof...(Exclude) == 0 || std::none_of(filter.cbegin(), filter.cend(), [entt](const basic_sparse_set<Entity> *cpool) { return cpool->contains(entt); })))
                {
                    if constexpr(std::is_invocable_v<Func, decltype(get<Type>({}))...>) {
                        func(get<Comp, Type>(it, std::get<view_type<Type> *>(pools), entt)...);
                    } else {
                        func(entt, get<Comp, Type>(it, std::get<view_type<Type> *>(pools), entt)...);
                    }
                }
            ...
            for(const auto entt: static_cast<const basic_sparse_set<entity_type> &>(*std::get<view_type<Comp> *>(pools))) {
                ...
                {
                    if constexpr(std::is_invocable_v<Func, decltype(get<Type>({}))...>) {
                        func(std::get<view_type<Type> *>(pools)->get(entt)...);
                    } else {
                        func(entt, std::get<view_type<Type> *>(pools)->get(entt)...);
                    }
                }
            }
        }
...
public:
    ...
    [[nodiscard]] size_type size_hint() const ENTT_NOEXCEPT {
        return (std::min)({ std::get<view_type<Component> *>(pools)->size()... });
    }
    ...
    [[nodiscard]] bool contains(const entity_type entt) const {
        return (std::get<view_type<Component> *>(pools)->contains(entt) && ...)
    ...
    [[nodiscard]] decltype(auto) get([[maybe_unused]] const entity_type entt) const {
        ...
        if constexpr(sizeof...(Comp) == 0) {
            ...
            return (std::get<view_type<Component> *>(pools)->get(entt), ...);
        } else if constexpr(sizeof...(Comp) == 1) {
            return (std::get<view_type<Comp> *>(pools)->get(entt), ...);
        } else {
    ...
    void each(Func func) const {
        view = candidate();
        ((std::get<view_type<Component> *>(pools) == view ? each<Component>(std::move(func)) : void()), ...);
    }
    ...
    template<typename Comp>
    [[nodiscard]] auto each() const ENTT_NOEXCEPT {
        const basic_sparse_set<entity_type> *cpool = std::get<view_type<Comp> *>(pools);
...
class basic_view<Entity, exclude_t<>, Component> {
    ...
    using view_type = view_t<Entity, Component>;
    ...
    class view_range {
        ...
        class range_iterator {
            ...
            using it_type = std::conditional_t<
                ...
                std::tuple<typename basic_sparse_set<Entity>::iterator, decltype(std::declval<view_type>().begin())>
            >;
    ...
    basic_view(view_type &ref) ENTT_NOEXCEPT
        : pool{&ref}
    {}
